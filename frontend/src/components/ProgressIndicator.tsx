...
export default function ProgressIndicator({ status }: { status: RenderStatus }) {
  ...
  const waiting = status.status === 'running' && status.progress <= 0
  const busy = status.status === 'running' && status.progress > 0
  return (
    <div className={`progress ${status.status === 'error' ? 'error' : ''} ${waiting ? 'waiting' : ''} ${busy ? 'busy' : ''}`}>
      <div className="bar">
      ...
      <div className="label">
        ...
        <span>{status.status === 'error' || waiting ? '' : `${percent}%`}</span>
      </div>
    </div>
  )
}
